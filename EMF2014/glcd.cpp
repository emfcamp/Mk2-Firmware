...
void glcd::setLCD(LCDTask lcdtask) {
	this->_lcdtask = lcdtask;
}
...
void glcd::InvertRect(uint8_t x, uint8_t y, uint8_t width, uint8_t height) {
	...
	glcd::GotoXY(x, y);
	for(i=0; i<=width; i++) {
		data = _lcdtask.ReadData();
		tmpData = ~data;
		data = (tmpData & mask) | (data & ~mask);
		_lcdtask.WriteData(data);
	}
	...
	if(h < height) {
		...
		for(i=0; i<=width; i++) {
			data = this->_lcdtask.ReadData();
			tmpData = ~data;
			data = (tmpData & mask) | (data & ~mask);
			this->_lcdtask.WriteData(data);
		}
	}
}
/**
...
#define BITMAP_FIX
  ...
  height += height % 8; //Force heght to multiple of 8 so you don't miss a row next
  ...
  for(j = 0; j < height / 8; j++) {
	 ...
	 for(i = 0; i < width; i++) {
		 uint8_t displayData = ReadPgmData(bitmap++);
	   	 if(color == BLACK)
			this->_lcdtask.WriteData(displayData);
		 else
		    this->_lcdtask.WriteData(~displayData);
	 }
  }
