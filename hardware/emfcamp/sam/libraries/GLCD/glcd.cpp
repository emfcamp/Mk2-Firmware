/*
  ...
  The functions were inspired from code written and copyright by Fabian
  Maximilian Thiele.
  ...
  It is derived from the glcd_Device class that impliments the protocol for
  sending and
 ...
 *
 * The optional @em invert parameter specifies if the display should be run in a
 *normal
 * mode, dark pixels on light background or inverted, light pixels on a dark
 *background.
 *
 * To specify dark pixels use the define @b NON-INVERTED and to use light pixels
 *use
...
int glcd::Init(uint8_t invert) { glcd_Device::Init(); }
 ...
 *
 * @note
 * If the display is in INVERTED mode, then the color WHITE will paint the
 *screen
...
void glcd::DrawRoundRect(uint8_t x, uint8_t y, uint8_t width, uint8_t height,
    ...
    this->DrawHLine(x + radius, y + height, width - (2 * radius),
                    color);                                       // bottom
    this->DrawVLine(x, y + radius, height - (2 * radius), color); // left
    this->DrawVLine(x + width, y + radius, height - (2 * radius),
                    color); // right
}
 ...
 *
 * The resulting rectangle covers an area @em width pixels wide by @em height
 *pixels
 ...
 *
 *
 * @warning FillRect() behavior has changed from the previous versions of the
 *ks0108 library.
 *	The filled rectangle will be one pixel smaller in width and height than
 *the old version.
...
void glcd::FillRect(uint8_t x, uint8_t y, uint8_t width, uint8_t height,
                    uint8_t color) {
 ...
 * @param height height of the rectangle
 *
 * Inverts the pixels in the rectanglular area of the specified width and
 *height.
 ...
 * @note The width and height parameters work differently than DrawRect()
 *
 * @warning InvertRect() behavior has changed from the previous versions of the
 *ks0108 library.
 *	The inverted rectangle will be one pixel smaller in width and height
...
#define BITMAP_FIX
     ...
     * are non multiples of 8 pixels in height and start on non LCD page Y
     *addresses.
     *
     ...
     *
     *	Currently, the low level WriteData() routine ORs in the pixels when data
     *spans
     *	a LCD memory page. So.....
     *
     * In order to ensure that the bitmap data is written to the pixels
     **exactly*
     *as it
     * it defined in the bitmap data, i.e every black pixels is black and every
     *white
     * pixel is white,...
     *
     * If height or y coordinate is not on a page boundary, clear the background
     *first
 ...
 * The glcd xbm bitmap data format consists of 1 byte of width followed by 1
 *byte of height followed
...
void glcd::DrawBitmapXBM(ImageXBM_t bitmapxbm, uint8_t x, uint8_t y,
                         uint8_t color) {
...
void glcd::DrawBitmapXBM_P(uint8_t width, uint8_t height, uint8_t *xbmbits,
                           uint8_t x, uint8_t y, uint8_t fg_color,
                           uint8_t bg_color) {
    uint8_t xbmx, xbmy;
    uint8_t xbmdata;
    ...
    for (xbmy = 0; xbmy < height; xbmy++) {
        for (xbmx = 0; xbmx < width; xbmx++) {
            ...
            if (xbmdata & _BV((xbmx & 7)))
                this->SetDot(x + xbmx, y + xbmy,
                             fg_color); // XBM 1 bits are fg color
            else
                this->SetDot(x + xbmx, y + xbmy,
                             bg_color); // XBM 0 bits are bg color
        }
    }
}
 ...
 *
 * color of BLACK or WHITE is an optional parameter indicating pixel color,
 *default is BLACK
 *
...
void glcd::DrawCircle(uint8_t xCenter, uint8_t yCenter, uint8_t radius,
                      uint8_t color) {
    this->DrawRoundRect(xCenter - radius, yCenter - radius, 2 * radius,
                        2 * radius, radius, color);
}
...
void glcd::FillCircle(uint8_t xCenter, uint8_t yCenter, uint8_t radius,
     ...
     * 		DrawRoundRect() first plots the cicrular quadrants and then
     *draws
     *		the horizontal and verticl lines between them to create
     *		a rounded rectangle.
     *
     *		For fills of rounded rectangles, this type of circle fill
     *algorithm
     *		would fill the upper and lower quadrants on each side of the
     *rounded
     *		rectangle by connecting them vertically.
     *		When that was done, a filled rectangle would be neded need to
     *fill
     *the
     *		space between them.
