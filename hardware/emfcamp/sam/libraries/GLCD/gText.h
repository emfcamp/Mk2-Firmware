...
// these should be replaced when Arduino supports FLASH strings in the base
// print class
#include <avr/pgmspace.h>
typedef class _FlashString {
} *FLASHSTRING;
 ...
 *
 * This allows the creatation of an unlmited number of predefined areas with
 *zero code or
 * data space overhead.
 *
 ...
 *
 * WARNING:
 *	This is non portable code in that it will only work on little endian
 *processors.
 *	If you use big endian you have to switch the byte ordering in the union.
 */

#define MK_TareaToken(x1, y1, x2, y2)                                          \
    (uint32_t)(((uint32_t)(x1) << 24) | ((uint32_t)(y1) << 16) |               \
...
typedef uint8_t textMode; // type holding mode for scrolling and future
                          // attributes like padding etc.
// the only textMode supported in the current release is scrolling

const textMode SCROLL_UP = 0;
const textMode SCROLL_DOWN =
    1; // this was changed from -1 so it can used in a bitmask
 ...
 * \ref gText::EraseTextLine( eraseLine_t type) "EraseTextLine()" function
 *call.\n
...
// static glcd_Device    *device;              // static pointer to the device
// instance
...
class gText : public glcd_Device {
  ...
  protected:
    ...
    void ScrollUp(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,
                  uint8_t pixels, uint8_t color);
    void ScrollDown(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,
                    uint8_t pixels, uint8_t color);

  public:
    gText(); // default - uses the entire display
    gText(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,
    ...
    gText(uint8_t x1, uint8_t y1, uint8_t columns, uint8_t rows, Font_t font,
          textMode mode = DEFAULT_SCROLLDIR);
    ...
    uint8_t DefineArea(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,
                       textMode mode = DEFAULT_SCROLLDIR);
    uint8_t DefineArea(uint8_t x1, uint8_t y1, uint8_t columns, uint8_t rows,
                       Font_t font, textMode mode = DEFAULT_SCROLLDIR);
    ...
    // Font Functions
    void SelectFont(Font_t font, uint8_t color = BLACK,
                    FontCallback callback = ReadPgmData); // default arguments
                                                          // added, callback now
                                                          // last arg
    ...
    void DrawString(String &str, uint8_t x,
                    uint8_t y); // for Arduino String class
    ...
    void CursorTo(uint8_t column, uint8_t row); // 0 based coordinates for
                                                // character columns and rows
    void CursorTo(int8_t column); // move cursor on the current row
    void CursorToXY(uint8_t x,
                    uint8_t y); // coordinates relative to active text area
    ...
    void EraseTextLine(eraseLine_t type =
                           eraseTO_EOL); // ansi like line erase function
    void EraseTextLine(uint8_t row); // erase the entire text line in the given
                                     // row and move cursor to left position

    // legacy text output functions
    void PrintNumber(long n);

#ifndef USE_ARDUINO_FLASHSTR
    // when the following function is supported in arduino it will be removed
    // from this library
    void printFlash(FLASHSTRING str); // this overrides the Arduino print
                                      // function to implicilty store the string
                                      // in flash (progmem)
