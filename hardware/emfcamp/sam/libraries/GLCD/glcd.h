 ...
 * may not be in the future since each is used to point to a different type of
 * data
 */
typedef const uint8_t *Image_t;    // a glcd format bitmap (includes width &
                                   // height)
typedef const uint8_t *ImageXBM_t; // a "xbm" format bitmap (includes width &
                                   // height)
...
class glcd : public gText {
  ...
  public:
    ...
    void SetDisplayMode(uint8_t mode); // NON_INVERTED or INVERTED,   was
                                       // SetInverted(uint8_t invert);
                                       /*@}*/
    ...
    void DrawRoundRect(uint8_t x, uint8_t y, uint8_t width, uint8_t height,
                       uint8_t radius, uint8_t color = BLACK);
    ...
    void DrawBitmap(Image_t bitmap, uint8_t x, uint8_t y,
                    uint8_t color = BLACK);
#ifdef NOTYET
    void DrawBitmapXBM(ImageXBM_t bitmapxbm, uint8_t x, uint8_t y,
                       uint8_t color = BLACK);
