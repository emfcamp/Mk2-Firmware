...
gText::gText(uint8_t x1, uint8_t y1, uint8_t columns, uint8_t rows, Font_t font,
             textMode mode) {
    // device = (glcd_Device*)&GLCD;
...
void gText::ClearArea(void) {
    ...
    */
    ...
    glcd_Device::SetPixels(this->tarea.x1, this->tarea.y1, this->tarea.x2,
                           this->tarea.y2,
                           this->FontColor == BLACK ? WHITE : BLACK);
    /*
    * put cursor at home position of text area to ensure we are always inside
    * area.
    */
...
*
* @note
* Upon defining the text area, the cursor position for the text area will be set
*to x,y
*
* @see ClearArea()
*/

uint8_t gText::DefineArea(uint8_t x, uint8_t y, uint8_t columns, uint8_t rows,
                          Font_t font, textMode mode) {
    uint8_t x2, y2;
...
* The pixel coordinates for the text area are inclusive so x2,y2 is the lower
*right
* pixel of the text area.
*
* x1,y1 and x2,y2 are an absolute coordinates and are relateive to the 0,0
*origin of the
* display.
*
* The area within the newly defined text area is intentionally not cleared.
...
*
* @note
* Upon creation of the text area, the cursor position for the text area will be
*set to x1, y1
*
* @see ClearArea()
*
...
uint8_t gText::DefineArea(predefinedArea selection, textMode mode) {
    uint8_t x1, y1, x2, y2;
    TareaToken tok;
    ...
    for (col = x1; col <= x2; col++) {
        ...
        */
        ...
        while (sy <= y2) {
            ...
            if ((dy & 7) == 7) {
                glcd_Device::GotoXY(col,
                                    dy & ~7); // Should be able to remove this
...
void gText::SpecialChar(uint8_t c) {
    if (c == '\n') {
        ...
        * It looks better when using inverted (WHITE) text, on proportional
        *fonts, and
        ...
        */
        ...
        if (this->x < this->tarea.x2) {
            glcd_Device::SetPixels(this->x, this->y, this->tarea.x2,
                                   this->y + height,
                                   this->FontColor == BLACK ? WHITE : BLACK);
        }
/*
...
*/
...
#endif
        {
            ...
            * This extra pixel is along the bottom for a "gap" between the
            * character below.
            */
            if (this->y + 2 * height >= this->tarea.y2) {
                ...
                *
                *		pixels = height - ((this->tarea.y2 - this->y)  -
                *height) +1;
                *
                *		The forumala below is unchanged
                *		But has been re-written/simplified in hopes of
                *better code
                *
                */
                ...
                * When less than the full character height of pixels is
                *scrolled,
                * There can be an issue with the newly created empty line.
                * This is because only the # of pixels scrolled will be colored.
                * What it means is that if the area starts off as white and the
                *text
                * color is also white, the newly created empty text line after a
                *scroll
                * operation will not be colored BLACK for the full height of the
                *character.
                * The only way to fix this would be alter the code use a "move
                *pixels"
                * rather than a scroll pixels, and then do a clear to end line
                *immediately
                * after the move and wrap.
                *
                * Currently this only shows up when
                * there are are less than 2xheight pixels below the current Y
                *coordinate to
                * the bottom of the text area
                * and the current background of the pixels below the current
                *text line
                * matches the text color
                * and  a wrap was just completed.
                *
                * After a full row of text is printed, the issue will resolve
                *itself.
                *
                *
                */
                this->ScrollUp(this->tarea.x1, this->tarea.y1, this->tarea.x2,
                               this->tarea.y2, pixels,
                               this->FontColor == BLACK ? WHITE : BLACK);
            ...
            */
            if (this->y > this->tarea.y1 + height) {
                ...
                */
                ...
                this->ScrollDown(this->tarea.x1, this->tarea.y1, this->tarea.x2,
                                 this->tarea.y2, pixels,
                                 this->FontColor == BLACK ? WHITE : BLACK);
...
int gText::PutChar(uint8_t c) {
    ...
    */
    ...
    uint8_t height = FontRead(this->Font + FONT_HEIGHT);
    uint8_t bytes =
        (height + 7) / 8; /* calculates height in rounded up bytes */
...
*
* @param str pointer to a null terminated character string stored in program
*memory
*
* Outputs all the characters in the string to the text area.
* See PutChar() for a full description of how characters are
...
void gText::CursorTo(uint8_t column, uint8_t row) {
    ...
    */
    ...
    this->x =
        column * (FontRead(this->Font + FONT_FIXED_WIDTH) + 1) + this->tarea.x1;
...
*
* If column is negative then the column position is relative to the current
*cursor
* position.
*
* @warning
* While intended only for fixed width fonts, cursor repositioning will be done
*for variable
* width fonts.
* When variable width fonts are used, the column is based on assuming a width
* of the widest character in the font.
* Because the widest character is used for the amount of cursor movement, the
*amount
* of cursor movement when using relative positioning will often not be
*consistent with
* the number characters previously rendered. For example, if a letter "l" was
*written
* and the cursor was reposisitioned with a -1, the amount backed up will be much
*larger
...
void gText::EraseTextLine(eraseLine_t type) {
    ...
    case eraseFROM_BOL:
        ...
        break;
    case eraseFULL_LINE:
        glcd_Device::SetPixels(this->tarea.x1, y, this->tarea.x2, y + height,
                               color);
        break;
    }
...
*
* Font definitions from included font definition files are stored in program
*memory
* You can have as many fonts defines as will fit in program memory up to 64k and
*can
...
void gText::SelectFont(Font_t font, uint8_t color, FontCallback callback) {
    this->Font = font;
    FontRead = callback; // this sets the callback that will be used by all
                         // instances of gText
...
void gText::SetFontColor(uint8_t color) { this->FontColor = color; }
...
void gText::SetTextMode(textMode mode) {
    ...
    /*
    * when other modes are added the tarea.mode variable will hold a bitmask or
    * enum for the modde and should be renamed
    */
...
* including any inter-character gap pixels following the character when the
*character is
...
void gText::PrintNumber(long n) {
    ...
    if (n == 0)
        ...
        for (; i > 0; i--)
            PutChar((char)(buf[i - 1] < 10 ? '0' + buf[i - 1]
                                           : 'A' + buf[i - 1] - 10));
    }
}
...
#if ARDUINO < 100
void gText::write(uint8_t c) { this->PutChar(c); }
#else
size_t gText::write(uint8_t c) { return (this->PutChar(c)); }
#endif

#ifndef USE_ARDUINO_FLASHSTR
// functions to store and print strings in Progmem
// these should be removed when Arduino supports FLASH strings in the base print
// class
/**
...
* @see Puts_P()
*/
void gText::printFlash(FLASHSTRING str) { Puts_P((PGM_P)str); }
...
*
* The Printf() functions fudge up a STDIO stream to point back to a C callable
*function
...
* After the @em format parameter, the function expects at least as many
*additional
...
* print formatted data
*
* @param format string in AVR progmem that contains text or optional embedded
*format tags
* @param ... Depending on the format string, the function may expect a sequence
*of additional arguments.
*
